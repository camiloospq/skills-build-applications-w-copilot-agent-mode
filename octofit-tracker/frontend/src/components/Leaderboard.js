...
const Leaderboard = () => {
  ...
  const columns = useMemo(() => (leaderboard.length ? Object.keys(leaderboard[0]) : []), [leaderboard]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return leaderboard;
    return leaderboard.filter((row) => JSON.stringify(row).toLowerCase().includes(q));
  }, [leaderboard, query]);
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-body">
        ...
        <form className="mb-3">
          <label className="form-label" htmlFor="leaderboardSearch">Filter</label>
        ...
        {loading ? (
          ...
          <>
            {filtered.length === 0 ? (
              <div className="alert alert-info">No entries to display.</div>
            ) : (
