...
const Users = () => {
  ...
  const columns = useMemo(() => (users.length ? Object.keys(users[0]) : []), [users]);
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-body">
        ...
        <form className="mb-3">
          <label className="form-label" htmlFor="usersSearch">Filter</label>
        ...
        {loading ? (
          ...
          <>
            {filtered.length === 0 ? (
              <div className="alert alert-info">No users to display.</div>
            ) : (
