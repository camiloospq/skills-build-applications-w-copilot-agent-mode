...
const Teams = () => {
  ...
  const columns = useMemo(() => (teams.length ? Object.keys(teams[0]) : []), [teams]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return teams;
    return teams.filter((row) => JSON.stringify(row).toLowerCase().includes(q));
  }, [teams, query]);
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-body">
        ...
        <form className="mb-3">
          <label className="form-label" htmlFor="teamsSearch">Filter</label>
        ...
        {loading ? (
          <div className="text-center py-5">
            ...
            <div className="mt-2 text-muted">Loading teams…</div>
          </div>
        ) : (
          <>
            {filtered.length === 0 ? (
              <div className="alert alert-info">No teams to display.</div>
            ) : (
